...
void TextEditor::Render()
{
	...
	char buf[16];
	snprintf(buf, 16, " %04d ", std::max(9999, globalLineMax));
	mTextStart = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, buf, nullptr, nullptr).x + mLeftMargin;

	// Draw Line number background over the entire scrollable content height
	float bgHeight = std::max(ImGui::GetWindowHeight(), (float)mLines.size() * mCharAdvance.y + 50.f);
	drawList->AddRectFilled(
		ImVec2(cursorScreenPos.x, cursorScreenPos.y),
		ImVec2(cursorScreenPos.x + mTextStart, cursorScreenPos.y + bgHeight),
		IM_COL32(24, 24, 24, 255));
	// Draw separator line
	drawList->AddLine(
		ImVec2(cursorScreenPos.x + mTextStart, cursorScreenPos.y),
		ImVec2(cursorScreenPos.x + mTextStart, cursorScreenPos.y + bgHeight),
		...
		while (lineNo <= lineMax)
		{
			...
			if (errorIt != mErrorMarkers.end())
			{
				...
				if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
				{
					...
					ImGui::PushStyleColor(ImGuiCol_PopupBg, IM_COL32(30, 15, 15, 255));
					ImGui::PushStyleColor(ImGuiCol_Border, IM_COL32(230, 40, 40, 255));

					ImGui::BeginTooltip();
					ImGui::TextColored(ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "Error at line %d:", errorIt->first);
					ImGui::Dummy(ImVec2(0, 4.f));
					ImGui::TextColored(ImVec4(0.9f, 0.9f, 0.9f, 1.0f), "%s", errorIt->second.c_str());
					ImGui::EndTooltip();
...
{
	// Perception-style palette (VS Code Dark+ inspired):
	//   purple keywords, red types, yellow functions, green numbers/comments,
	//   light blue identifiers, orange strings. Format is ABGR (ImU32/IM_COL32).
	const static Palette p = { {
			0xffd4d4d4,	// Default              — light gray (fallback text)
			0xffc586c0,	// Keyword              — purple  (class/if/return/void/…)
			0xffb5cea8,	// Number               — light green (0, 800.0f, false…)
			0xff7891ce,	// String               — orange  ("hello")
			0xff7891ce, // Char literal          — orange  ('a')
			0xffd4d4d4, // Punctuation           — light gray ({ } ( ) ; ,)
			0xffc586c0,	// Preprocessor          — purple  (#include etc)
			0xff9cdcfe, // Identifier            — light blue (variables x, y, dt…)
			0xff7b7bff, // Known identifier      — red     (types: float, bool, uint8)
			0xffaadcdc, // Preproc identifier    — yellow  (function names, macros)
			0xff55996a, // Comment (single line) — green   (// …)
			0xff55996a, // Comment (multi line)  — green   (/* … */)
			0xff0f0f0f, // Background            — matches menu C_BG
			0xffe0e0e0, // Cursor                — near-white
			0x50b47850, // Selection             — soft blue tint
			0x805050ff, // ErrorMarker           — translucent red
			0x405078ff, // Breakpoint            — translucent red-orange
			0xff6e6e6e, // Line number           — dim gray
			0x14ffffff, // Current line fill     — subtle white overlay
			0x08ffffff, // Current line fill (inactive)
			0x00000000, // Current line edge     — no border
		} };
	return p;
}
